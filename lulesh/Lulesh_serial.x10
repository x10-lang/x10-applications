...
*/
    	val iteration = "Iteration "+iter;
	//val content = "";
    	outputHandle.write(iteration.bytes());
    	for(var index:int = 0n; index < domain.numElem(); index++){
    		
    		//val content = "x=" + Domain.m_x(index) + " y="+ Domain.m_y(index) + " z=" + Domain.m_z(index) + " fx=" + Domain.m_fx(index) + " fy=" + Domain.m_fy(index) + " fz=" + Domain.m_fz(index);
    		val content = Domain_serial.m_x(index);

    		val a = "x= " + Domain_serial.m_y(index);
    		val b= Domain_serial.m_z(index);
    		val c = Domain_serial.m_fx(index);
    		val d = Domain_serial.m_fy(index);
    		val e = Domain_serial.m_fz(index);
    		outputHandle.writeDouble(content);
    		/*
    		outputHandle.write("x=".bytes());
    		outputHandle.writeDouble(domain.x(index));
    		outputHandle.write("y=".bytes());
    		outputHandle.writeDouble(domain.y(index));
    		outputHandle.write("z=".bytes());
    		outputHandle.writeDouble(domain.z(index));
    		 */
    	}
