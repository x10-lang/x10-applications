...
static double mult_x_time = 0;
static double mult_y_time = 0;
static double mult_z_time = 0;
static double mult_t_time = 0;
...
void opr_H(double *v, double *u, double *w){
  ...
  // debug
  double mult_start_time = gettimeofday_sec();
  mult_xp(v,u,w);
  mult_xm(v,u,w);
  mult_x_time += gettimeofday_sec() - mult_start_time;
  double mult_y_start_time = gettimeofday_sec();
  mult_yp(v,u,w);
  mult_ym(v,u,w);
  mult_y_time += gettimeofday_sec() - mult_y_start_time;
  double mult_z_start_time = gettimeofday_sec();
  mult_zp(v,u,w);
  mult_zm(v,u,w);
  mult_z_time += gettimeofday_sec() - mult_z_start_time;
  double mult_t_start_time = gettimeofday_sec();
  mult_tp(v,u,w);
  mult_tm(v,u,w);
  mult_t_time += gettimeofday_sec() - mult_t_start_time;
  int id1 = 0;
  int id2 = Nvc;
  int id3 = Nvc*2;
  int id4 = Nvc*3;
...
void mult_xp(double *v2, double *u, double *v1){
  ...
  // boundary part 
    ix = Nx-1;
    nn = 0;
#pragma omp parallel for private( vt1,vt2,wt1r,wt1i,wt2r,wt2i )
