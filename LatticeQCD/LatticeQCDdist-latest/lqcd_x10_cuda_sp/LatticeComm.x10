...
class LatticeComm extends ParallelLattice{
	...
	val refBuffers : PlaceLocalHandle[Rail[GlobalRail[Float]]];
...
//debug
		...
		refBuffers = PlaceLocalHandle.make[Rail[GlobalRail[Float]]](Place.places(), ()=>new Rail[GlobalRail[Float]](8));
...
//debug
					refBuffers()(i) = at(Place(iDest)) new GlobalRail[Float](bufRecv(i).v());
					// refBuffers()(i) = at(Place(neighbors()(i))) new GlobalRail[Float](bufRecv(i).v());
				}
			}
			refInitDone = 1;
		}
	...
	def Send(dir : Long)
	{
	  if(decomp(dir) > 1){
		...
		finish{
		  Rail.asyncCopy[Float](bufRef,0,bufRecv(dir).v(),0,size);
		}
		// finish{
		  // Rail.uncountedCopy[Float](bufRef, 0, bufRecv(dir).v(), 0, size, ()=>{recvCount()(dir)+=size;});
		// }
	      }
	    }
	  }
...
//debug
	  Rail.asyncCopy[Float](bufSend(dir).v(), 0, refBuffers()(dir), 0, size);
	  // Rail.uncountedCopy[Float](bufSend(dir).v(), 0, refBuffers()(dir), 0, size, ()=>{
	  //   atomic counts()(dir) += 1;
